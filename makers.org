...
** TODO [#A] fix equipment type dropdown in one_event, and the thing that receives it (de-pretty the type name
** TODO [#A] fix output and input of durations
...
** OPEN [#B] training success to cancel training requests
   :PROPERTIES:
